-- [[ UNIFIED MOBILE SCRIPT V2 (FIXED & FULL COPIED) ]]
-- Make sure to copy from this line down to the VERY LAST line with parentheses!
...
local function CreateHighlight(parent, color)
    local hl = parent:FindFirstChild("VENOM_HL")
    if not hl then
...
local function UpdateESP()
    ...
    if not ESP.Enabled then
        for vehicle, data in pairs(ESP.Instances) do
            if data.HullHighlight then data.HullHighlight.Enabled = false end
            if data.TurretHighlight then data.TurretHighlight.Enabled = false end
        end
        return
    end
    ...
    for _, vehicle in ipairs(vehicles:GetChildren()) do
        if vehicle:IsA("Model") and not vehicle.Name:find(LocalPlayer.Name) then
            ...
            local current = ESP.Instances[vehicle]
            
            local hull = vehicle:FindFirstChild("Hull")
            if hull and (not current.HullHighlight or current.HullHighlight.Parent ~= hull) then
                current.HullHighlight = CreateHighlight(hull, ESP.HullColor)
            end
            
            local turret = vehicle:FindFirstChild("Turret")
            if turret and (not current.TurretHighlight or current.TurretHighlight.Parent ~= turret) then
                current.TurretHighlight = CreateHighlight(turret, ESP.TurretColor)
            end
            
            -- Apply real-time updates based on toggles
            local hasVisuals = ESP.EnableFill or ESP.EnableOutline
            if current.HullHighlight then
                current.HullHighlight.Enabled = ESP.Enabled and hasVisuals
                current.HullHighlight.FillTransparency = ESP.EnableFill and ESP.FillTransparency or 1
                current.HullHighlight.OutlineTransparency = ESP.EnableOutline and ESP.OutlineTransparency or 1
            end
            if current.TurretHighlight then
                current.TurretHighlight.Enabled = ESP.Enabled and hasVisuals
                current.TurretHighlight.FillTransparency = ESP.EnableFill and ESP.FillTransparency or 1
                current.TurretHighlight.OutlineTransparency = ESP.EnableOutline and ESP.OutlineTransparency or 1
            end
        end
    end
    
    -- Clean up removed vehicles
    for vehicle, _ in pairs(ESP.Instances) do
        if not vehicle.Parent then ESP.Instances[vehicle] = nil end
    end
end
...
local themes = {
    ...
    Accent = Color3.fromRGB(255, 60, 60), -- Changed to red for better contrast in console
...
function Library.new(title)
    local sg = Instance.new("ScreenGui")
    sg.Name = "VENOM_MOBILE_GUI"
    sg.ResetOnSpawn = false
    -- Try CoreGui first, fallback to PlayerGui for Mobile Exploits
    ...
    closeBtn.TextColor3 = themes.Accent
...
---------------------------------------------------------
-- UI BUILD SETUP & EXECUTION
---------------------------------------------------------
local win = Library.new("CTS/VENOM (MOBILE)")
