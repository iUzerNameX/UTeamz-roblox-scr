...
-- LOAD UI LIBRARY
---------------------------------------------------------
local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/iUzerNameX/UTeamz-roblox-scr/main/UILibrary.lua"))()
...
function SaveConfig()
    local data = {
        ESP = {
            ...
            HullColor = {R = ESP.HullColor.R, G = ESP.HullColor.G, B = ESP.HullColor.B},
            TurretColor = {R = ESP.TurretColor.R, G = ESP.TurretColor.G, B = ESP.TurretColor.B}
        },
        Mark = {
...
if getgenv then getgenv().SaveConfig = SaveConfig else _G.SaveConfig = SaveConfig end
    ...
    if readfile and isfile and isfile(ConfigFileName) then
        ...
        if success and type(result) == "table" then
            if result.ESP then
                ...
                if result.ESP.HullColor then 
                    ESP.HullColor = Color3.new(result.ESP.HullColor.R, result.ESP.HullColor.G, result.ESP.HullColor.B) 
                end
                if result.ESP.TurretColor then 
                    ESP.TurretColor = Color3.new(result.ESP.TurretColor.R, result.ESP.TurretColor.G, result.ESP.TurretColor.B) 
                end
            end
            if result.Mark then
...
Services.RunService.RenderStepped:Connect(function(dt)
    ...
    if Fly.Active then
        if not Fly.Root or not Fly.Root.Parent or not bv.Parent then
            initFlyRoot()
            if Fly.Root then
                bv.Parent = Fly.Root
                bg.Parent = Fly.Root
...
-- UI INTERFACE BUILD
---------------------------------------------------------
win = Library.new("UTeamz | t.me/UTeamz")
win:Notify({Title = "System", Content = "Script successfully injected!", Duration = 5})

local espTab = win:AddTab("ESP")
UI_Elements.ESP_Enabled = espTab:AddToggle("Enable ESP", ESP.Enabled, function(v)
    ESP.Enabled = v
    UpdateAllESPInstances()
end)
UI_Elements.ESP_TeamCheck = espTab:AddToggle("Team Check", ESP.TeamCheck, function(v)
    ESP.TeamCheck = v
    ClearAllESP()
    ScanVehicles()
end)
UI_Elements.ESP_ShowDistance = espTab:AddToggle("Show Distance", ESP.ShowDistance, function(v)
    ESP.ShowDistance = v
    UpdateAllESPInstances()
end)
UI_Elements.ESP_EnableFill = espTab:AddToggle("Enable Fill", ESP.EnableFill, function(v)
    ESP.EnableFill = v
    UpdateAllESPInstances()
end)
UI_Elements.ESP_EnableOutline = espTab:AddToggle("Enable Outline", ESP.EnableOutline, function(v)
    ESP.EnableOutline = v
    UpdateAllESPInstances()
end)

espTab:AddColorPicker("Hull Color", ESP.HullColor, function(c)
    ESP.HullColor = c
    UpdateAllESPInstances()
end)
espTab:AddColorPicker("Turret Color", ESP.TurretColor, function(c)
    ESP.TurretColor = c
    UpdateAllESPInstances()
end)
...
UI_Elements.Fly_Active = flyTab:AddToggle("Enable Fly", Fly.Active, function(v)
    SetFlyState(v)
end)
flyTab:AddSlider("Fly Speed", 10, 300, Fly.Speed, function(v) Fly.Speed = v end)
UI_Elements.Fly_MoveUp = flyTab:AddToggle("Move Up", false, function(v) Fly.MoveUp = v end)
UI_Elements.Fly_MoveDown = flyTab:AddToggle("Move Down", false, function(v) Fly.MoveDown = v end)
UI_Elements.Fly_MoveFwd = flyTab:AddToggle("Move Forward", false, function(v) Fly.MoveFwd = v end)
UI_Elements.Fly_MoveBwd = flyTab:AddToggle("Move Backward", false, function(v) Fly.MoveBwd = v end)

local bindsTab = win:AddTab("Binds")
bindsTab:AddKeybind("Toggle ESP", Enum.KeyCode.RightAlt, function()
    ESP.Enabled = not ESP.Enabled
    if UI_Elements.ESP_Enabled then UI_Elements.ESP_Enabled:Set(ESP.Enabled, true) end
    UpdateAllESPInstances()
    win:Notify({Title = "Keybind", Content = "ESP is now: " .. tostring(ESP.Enabled), Duration = 2})
end)

bindsTab:AddKeybind("Toggle Fly", Enum.KeyCode.F, function()
    local newState = not Fly.Active
    SetFlyState(newState)
    if UI_Elements.Fly_Active and UI_Elements.Fly_Active.Set then 
        UI_Elements.Fly_Active:Set(newState, true) 
    end
    win:Notify({Title = "Keybind", Content = "Fly is now: " .. tostring(newState), Duration = 2})
end)

bindsTab:AddKeybind("Toggle PenView", Enum.KeyCode.P, function()
    Other.PenView = not Other.PenView
    if UI_Elements.Other_PenView then UI_Elements.Other_PenView:Set(Other.PenView, true) end
...
configTab:AddButton("Load Config (CTSUteamz)", function()
    if LoadConfig() then
        if UI_Elements.ESP_Enabled then UI_Elements.ESP_Enabled:Set(ESP.Enabled, true) end
        if UI_Elements.ESP_TeamCheck then UI_Elements.ESP_TeamCheck:Set(ESP.TeamCheck, true) end
        if UI_Elements.ESP_ShowDistance then UI_Elements.ESP_ShowDistance:Set(ESP.ShowDistance, true) end
        if UI_Elements.ESP_EnableFill then UI_Elements.ESP_EnableFill:Set(ESP.EnableFill, true) end
        if UI_Elements.ESP_EnableOutline then UI_Elements.ESP_EnableOutline:Set(ESP.EnableOutline, true) end
        if UI_Elements.Mark_Enabled then UI_Elements.Mark_Enabled:Set(Mark.Enabled, true) end
        if UI_Elements.Other_PenView then UI_Elements.Other_PenView:Set(Other.PenView, true) end
        if UI_Elements.Other_RemoveFog then UI_Elements.Other_RemoveFog:Set(Other.RemoveFog, true) end
        if UI_Elements.Fly_Active then UI_Elements.Fly_Active:Set(Fly.Active, true) end
...
infoTab:AddButton("Copy Telegram Link", function()
    if setclipboard then
        ...
        win:Notify({Title = "Success", Content = "Copied to clipboard!", Duration = 3})
    end
end)
